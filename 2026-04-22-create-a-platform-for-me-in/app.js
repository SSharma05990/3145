...
const DEFAULT_MANAGER_PIN = "2805";
const DEFAULT_EMPLOYEE_PIN = "3145";
...
function normalizeAccess(access = {}) {
  return {
    managerPin: String(access.managerPin || DEFAULT_MANAGER_PIN),
    employeePin: String(access.employeePin || DEFAULT_EMPLOYEE_PIN),
...
function loginUser(event) {
  ...
  const pin = String(refs.loginPin.value || "").trim();
  const validPin = role === "manager" ? state.access.managerPin : state.access.employeePin;

  if (pin !== validPin) {
    refs.loginMessage.textContent = "Incorrect PIN. Please try again.";
    return;
  }

  state.access.isAuthenticated = true;
  state.access.currentRole = role;
...
function renderAccess() {
  ...
  refs.sessionRoleLabel.textContent = loggedIn
    ? `${isManager() ? "Manager" : "Employee"} logged in`
    : "Not logged in";
